...
expName = 'map'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/tiffanychien/Documents/_vs260a/blind-spot-psychophysics/experiment/experiment.py',
...
trials = data.TrialHandler(nReps=5, method='random', 
    ...
    trialList=data.importConditions('conditions/top_line_locs.csv'),
...
for thisTrial in trials:
    ...
    bottom_line.setPos((-2*int(expInfo['eye_lr']),-4+jitter))
    middle_line.setPos((-2*int(expInfo['eye_lr']), jitter))
    top_line.setPos((-2*int(expInfo['eye_lr']), 4+loc+jitter))
    blind_spot.setPos((-2*int(expInfo['eye_lr']), -2))
    # keep track of which components have finished
    ...
    while continueRoutine:
        ...
        # *middle_line* updates
        if middle_line.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *top_line* updates
        if top_line.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
